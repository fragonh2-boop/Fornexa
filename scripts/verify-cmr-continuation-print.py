...
def verify(browser,out,name,rows,long_row,min_pages,exact_pages=None):
    ...
    joined='\n'.join(texts); missing=[i for i in range(1,rows+1) if not re.search(rf'\bMK-{i}\b',joined)]
...
def main():
    ...
    for r in results: print(f"{r['name']}: rows={r['rows']} pages={r['pages']} goods_pages={r['goods_pages']} sha256={r['sha256']}")
