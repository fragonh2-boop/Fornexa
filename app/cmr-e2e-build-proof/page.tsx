...
export default async function CmrE2eBuildProofPage() {
  ...
  const base = {
    source: "e2e-build-proof",
    viaje: "VJ-E2E-BUILD-001",
...
async function issue(payload: Record<string, unknown>) {
  ...
  if (!response.ok) throw new Error(`POST /api/cmr ${response.status}: ${JSON.stringify(body)}`);
  return body as { id: string; cmrNumber: string; cmrKey: string };
}
...
async function prove(issued: { id: string; cmrNumber: string; cmrKey: string }) {
  ...
  return {
    ...
    projected: {
      ...
      canonicalParties: detail?.canonical?.parties?.length ?? null,
      canonicalGoods: detail?.canonical?.goodsLines?.length ?? null,
      canonicalAttachments: detail?.canonical?.attachments?.length ?? null,
      canonicalClauses: detail?.canonical?.clauses?.length ?? null,
      sender: detail?.document?.sender ?? null,
      firstGoodsDescription: detail?.document?.metadata?.cmr?.goodsLines?.[0]?.description ?? null,
      firstAdrUn: detail?.document?.metadata?.cmr?.goodsLines?.[0]?.adr?.unNumber ?? null,
    },
    qr: {
      status: qrResponse.status,
      contentType: qrResponse.headers.get("content-type"),
      bytes: Buffer.byteLength(qrText),
      containsMobileRoute: qrText.includes("/api/mobile/cmr/"),
    },
  };
}
