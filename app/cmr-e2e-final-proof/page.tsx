...
export default async function CmrE2eFinalProofPage() {
  ...
  const payload = {
    ...
    successiveCarriers: [
      { name: "Transport Sucesivo E2E SARL", taxId: "FR-E2E-001", address: "Paris, France", country: "FR" }
    ]
  };
  ...
  try {
    ...
    const cmrId = String(issued.id);
    const cmrNumber = String(issued.cmrNumber);
    const cmrKey = String(issued.cmrKey);
    ...
    for (const result of [document, parties, goods, attachments, clauses, stops, events, bridge]) {
      if (result.error) throw result.error;
    }

    const detailRequest = new Request(`${ORIGIN}/api/cmr/${encodeURIComponent(cmrNumber)}`, {
      headers: { "x-fornexa-key": cmrKey }
    });
    const detailResponse = await getCmr(detailRequest, { params: Promise.resolve({ cmr: cmrNumber }) });
    const detail = await detailResponse.json();

    const qrRequest = new Request(`${ORIGIN}/api/cmr/${encodeURIComponent(cmrNumber)}/qr?key=${encodeURIComponent(cmrKey)}`);
    ...
    const proof = { ok: false, error: serializeError(error) };
    console.log("CMR_E2E_FINAL", JSON.stringify(proof));
    return <pre id="cmr-e2e-final-proof">{JSON.stringify(proof, null, 2)}</pre>;
  }
}
