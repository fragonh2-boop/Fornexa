...
export async function GET(request: Request) {
  ...
  try {
    const first = await issue(url.origin, base); created.push(first.id); const firstCheck = await inspect(supabase, url.origin, first);
    const second = await issue(url.origin, { ...base, viaje: "VJ-E2E-002", expedicion: expedition?.code ?? undefined, senderInstructions: "Segunda prueba con expedición real" }); created.push(second.id); const secondCheck = await inspect(supabase, url.origin, second);
...
async function inspect(supabase: ReturnType<typeof createSupabaseAdmin>, origin: string, issued: { id: string; cmrNumber: string; cmrKey: string }) {
  ...
  const detailResponse = await fetch(`${origin}/api/cmr/${encodeURIComponent(issued.cmrNumber)}`, { headers: { "x-fornexa-key": issued.cmrKey }, cache: "no-store" }); const detail = await detailResponse.json();
  ...
  const viewResponse = await fetch(`${origin}/dashboard/epod-cmr/${encodeURIComponent(issued.cmrNumber)}?key=${encodeURIComponent(issued.cmrKey)}`, { cache: "no-store" }); const viewHtml = await viewResponse.text();
