...
export default async function CustomersPage({ searchParams }: { searchParams: Promise<{ estado?: string }> }) {
  ...
  const stats = [
    ...
    [String(customers.filter(customer => customer.status === "En revisión").length), "En revisión", "En revisión"], [String(customers.reduce((total, customer) => total + customer.openOffers, 0)), "Ofertas abiertas", ""],
  ] as const;
  const columns = [{key:"code",label:"Código"},{key:"tradeName",label:"Cliente"},{key:"taxId",label:"NIF/CIF"},{key:"location",label:"Provincia / población"},{key:"segment",label:"Segmento"},{key:"addresses",label:"Direcciones"},{key:"shipments",label:"Expediciones"},{key:"revenue",label:"Facturación"},{key:"manager",label:"Responsable"},{key:"status",label:"Estado"}];
  const rows = filtered.map(customer => ({ code:customer.code, tradeName:customer.tradeName, taxId:customer.taxId, location:`${customer.province} · ${customer.city}`, segment:customer.segment, addresses:customer.addresses, shipments:customer.shipments, revenue:customer.annualRevenue, manager:customer.accountManager, status:customer.status }));
