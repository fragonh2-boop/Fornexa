  ...
  return <main className={styles.page}>
    ...
    <form id="party-form" className={styles.form} onSubmit={save}>
      <section className={styles.card}><div className={styles.cardTitle}><span>01</span><div><h2>Identificación y clasificación</h2><p>Información legal, comercial y operativa del tercero.</p></div></div><div className={styles.grid4}>
        <label>País fiscal<select name="country" value={partyCountry} onChange={event => { setPartyCountry(event.target.value as CountryCode); setTaxId(""); }}><option value="ES">España</option><option value="FR">Francia</option><option value="PT">Portugal</option></select><Check valid={true} hint="Determina formatos y catálogos" /></label><label>Código<input value={code} readOnly /></label><label>Tipo<select name="partyType" defaultValue="Cliente"><option>Cliente</option><option>Proveedor</option><option>Cliente y proveedor</option></select></label><label>{countryRules.taxLabel}<input name="taxId" value={taxId} onChange={event => setTaxId(event.target.value.toUpperCase())} required className={validationClass(taxValid)} /><Check valid={taxValid} hint={countryRules.taxHint} /></label>
