...
import { cmrDraftReadiness, createEmptyCmrDraft, customerIdsFromCmrSources, parseCustomerIds, type CmrDraft } from "@/lib/cmr-draft-readiness";
...
function detailsFromPartidas(lines:StoredPartida[]):StopDetailDraft[]{
 const first=lines[0];
 const orders=lines.map(line=>({id:line.id,customerId:customerIdsFromCmrSources([line])[0],description:line.mercancia||line.descripcion,packages:line.bultos,weight:line.peso}));
 return [
...
export default function NuevoCmrPage(){
 ...
 const [customerInput,setCustomerInput]=useState(initial.customerIds.join(", "));
 ...
 function setAdr(value:string){if(locked)return;setForm(current=>value==="S"?{...current,adr:value}:{...current,adr:value,adrRegime:"",unNumber:"",adrClass:"",packingGroup:"",tunnelCode:"",adrDescription:""});setNotice("")}
 function inheritFromExpeditions(expeditionIds:string[]){if(locked)return;try{
  ...
  const customerIds=customerIdsFromCmrSources([...selected,...lines]);
  setCustomerInput(customerIds.join(", "));
 ...
 function applyExpeditionInput(){const refs=parseExpeditionRefs(expeditionInput);setExpeditionInput(refs.join(", "));inheritFromExpeditions(refs)}
 ...
 async function emit(){
  ...
  try{
   const expeditionRefs=parseExpeditionRefs(expeditionInput);
   const communicatedStops=stopDetails.map(detail=>({...detail,fullAddress:detail.sequence===1?form.carga:form.entrega}));
   const payload={...form,expedicion:expeditionRefs[0]||undefined,expediciones:expeditionRefs,goodsLines:sourceLines.length?goodsLinesFromPartidas(sourceLines,form):undefined,stopDetails:communicatedStops};
   ...
   setIssued(result);setNotice(`${result.cmrNumber} emitido · ${expeditionRefs.length||result.expeditionIds?.length||0} expedición${expeditionRefs.length===1?"":"es"} vinculada${expeditionRefs.length===1?"":"s"}. La CMR Key y el QR ya están disponibles.`);
   const current=JSON.parse(localStorage.getItem("fornexa-cmr-documents")||"[]");
   localStorage.setItem("fornexa-cmr-documents",JSON.stringify([{id:result.cmrNumber,...form,expediciones:expeditionRefs,stopDetails:communicatedStops,status:"Emitido",cmrKey:result.cmrKey,createdAt:result.issuedAt},...(Array.isArray(current)?current:[])]));
 ...
 return <main className={styles.page}>
  ...
  <fieldset className={styles.lockable} disabled={locked}>
   <section className={styles.card}><div className={styles.sectionTitle}><span>1</span><div><h2>Documento de origen</h2><p>El CMR es una proyección documental: puede vincular una o varias expediciones sin fusionar su identidad operativa.</p></div></div><div className={styles.grid3}><label>Crear desde<select value={form.source} onChange={e=>set("source",e.target.value)}><option value="expedicion">Expedición</option><option value="viaje">Viaje</option></select></label><label>Expediciones <small>(separadas por coma)</small><input value={expeditionInput} onChange={e=>setExpeditionInput(e.target.value)} onBlur={applyExpeditionInput} onKeyDown={e=>{if(e.key==="Enter"){e.preventDefault();applyExpeditionInput()}}} placeholder="EX-260071, EX-260072"/></label><label>Viaje / tramo documental<input value={form.viaje} onChange={e=>set("viaje",e.target.value)}/></label><label>Customer ID maestro <small>(separados por coma)</small><input value={customerInput} onChange={e=>{setCustomerInput(e.target.value);set("customerIds",parseCustomerIds(e.target.value))}} placeholder="CL-0001, CL-0002"/></label></div></section>
   ...
   <section className={styles.card}><div className={styles.sectionTitle}><span>4</span><div><h2>Mercancía</h2><p>Los totales se agregan para el documento y cada partida se conserva como línea identificada al emitir.</p></div></div><div className={styles.grid3}><label>Descripción<input value={form.mercancia} onChange={e=>set("mercancia",e.target.value)}/></label><label>Número de bultos<input value={form.bultos} onChange={e=>set("bultos",e.target.value)}/></label><label>Emalaje<input value={form.embalaje} onChange={e=>set("embalaje",e.target.value)}/></label><label>Peso bruto (kg)<input value={form.peso} onChange={e=>set("peso",e.target.value)}/></label><label>Volumen (m³)<input value={form.volumen} onChange={e=>set("volumen",e.target.value)}/></label><label>Instrucciones<input value={form.instrucciones} onChange={e=>set("instrucciones",e.target.value)}/></label></div></section>
   <section className={styles.card}><div className={styles.sectionTitle}><span>5</span><div><h2>ADR</h2><p>El resumen se muestra aquí; al emitir, la información ADR se mantiene por línea de mercancía.</p></div></div><div className={styles.grid3}><label>ADR<select value={form.adr} onChange={e=>setAdr(e.target.value)}><option value="">Sin declarar</option><option value="N">No</option><option value="S">Sí</option></select></label><label>Régimen<input value={form.adrRegime} onChange={e=>set("adrRegime",e.target.value)} placeholder="1.1.3.6"/></label><label>Número ONU<input value={form.unNumber} onChange={e=>set("unNumber",e.target.value)} placeholder="UN 1263"/></label><label>Clase<input value={form.adrClass} onChange={e=>set("adrClass",e.target.value)} placeholder="3"/></label><label>Grupo embalaje<input value={form.packingGroup} onChange={e=>set("packingGroup",e.target.value)} placeholder="II"/></label><label>Código túnel<input value={form.tunnelCode} onChange={e=>set("tunnelCode",e.target.value)} placeholder="D/E"/></label><label>Designación ADR<input value={form.adrDescription} onChange={e=>set("adrDescription",e.target.value)} placeholder="Designación oficial de transporte"/></label></div></section>
  </fieldset>
