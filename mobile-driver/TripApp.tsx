...
export default function TripApp({ onExit }: { onExit: () => void }) {
  ...
  const stops = useMemo<TripStop[]>(() => {
    if (!trip) return [];
    return trip.expeditions.flatMap((expedition) =>
  ...
  const loadTrip = useCallback(async (value = normalizedToken, quiet = false) => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/mobile/trips/${encodeURIComponent(accessToken)}`, {
  ...
  async function postEvent(stop: TripStop, type: "arrival" | "complete") {
    try {
      setLoading(true);
      ...
      Alert.alert("Sincronizado", type === "arrival" ? "Llegada registrada." : "Parada completada.");
    } catch (error) {
  ...
  async function openRoute() {
    ...
    try {
      ...
      const origin = points[0], destination = points.at(-1);
  ...
  async function finishTrip() {
    try {
      setLoading(true);
      const response = await fetch(`${API_URL}/api/mobile/trips/${encodeURIComponent(token)}/finish`, {
        method: "POST",
        headers: { "x-idempotency-key": `finish-trip-${Date.now()}` },
      });
      const body = await response.json();
  ...
  if (scanner) {
    if (!permission?.granted) {
      return <SafeAreaView style={styles.safe}><View style={styles.center}><Text style={styles.title}>Acceso a cámara</Text><Text style={styles.muted}>Necesitamos la cámara para leer el QR del Viaje.</Text><Button label="Permitir cámara" onPress={requestPermission} /><Button label="Volver" tone="secondary" onPress={() => setScanner(false)} /></View></SafeAreaView>;
    }
    return <SafeAreaView style={styles.safe}><CameraView style={StyleSheet.absoluteFill} barcodeScannerSettings={{ barcodeTypes: ["qr"] }} onBarcodeScanned={({ data }) => loadTrip(data)} /><View style={styles.scannerOverlay}><Text style={styles.scannerTitle}>Escanea el QR del Viaje</Text><View style={styles.scanFrame} /><Button label="Cancelar" tone="secondary" onPress={() => setScanner(false)} /></View></SafeAreaView>;
  }

  if (!trip) {
    return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.home}><TouchableOpacity onPress={onExit}><Text style={styles.back}>‹ Volver</Text></TouchableOpacity><Text style={styles.eyebrow}>VIAJE ASIGNADO</Text><Text style={styles.title}>Importa tu viaje</Text><Text style={styles.muted}>Un único acceso carga todos los expedientes, CMR y paradas que forman parte del viaje.</Text><Button label="Escanear QR de Viaje" onPress={() => setScanner(true)} /><TextInput value={token} onChangeText={setToken} autoCapitalize="none" autoCorrect={false} placeholder="Token de viaje" style={styles.input} onSubmitEditing={() => loadTrip()} /><Button label={loading ? "Importando…" : "Importar viaje"} tone="secondary" onPress={() => loadTrip()} />{loading && <ActivityIndicator />}</ScrollView></SafeAreaView>;
  }

  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.page} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); loadTrip(token, true); }} />}><View style={styles.top}><TouchableOpacity onPress={() => setTrip(null)}><Text style={styles.back}>‹ Cambiar viaje</Text></TouchableOpacity><Text style={styles.sync}>● Sincronizado</Text></View><Text style={styles.eyebrow}>VIAJE · {trip.trip.status}</Text><Text style={styles.title}>{trip.trip.code}</Text><Text style={styles.muted}>{trip.trip.vehicle?.registration || "Sin vehículo"} · {trip.trip.driver?.name || "Sin conductor"}{trip.trip.trailer_registration ? ` · Remolque ${trip.trip.trailer_registration}` : ""}</Text><View style={styles.summary}><Summary label="Expediciones" value={String(trip.expeditions.length)} /><Summary label="Paradas pendientes" value={String(pending.length)} /><Summary label="Finalizadas" value={String(completed.length)} /></View><Button label="Proyectar ruta completa" onPress={openRoute} />{pending.length === 0 && completed.length > 0 && trip.trip.status !== "COMPLETED" && <Button label="Finalizar viaje" onPress={() => Alert.alert("Finalizar viaje", "Todas las paradas están completadas. ¿Cerrar el viaje?", [{ text: "Cancelar" }, { text: "Finalizar", onPress: finishTrip }])} />}<Text style={styles.section}>Paradas</Text>{stops.map((stop, index) => <View key={stop.id} style={[styles.card, stop.status === "Completada" && styles.cardDone]}><View style={styles.cardTop}><View style={styles.number}><Text style={styles.numberText}>{index + 1}</Text></View><View style={styles.cardBody}><Text style={styles.stopType}>{stop.stop_type} · {stop.expeditionCode}</Text><Text style={styles.company}>{stop.company}</Text><Text style={styles.meta}>{stop.address}</Text><Text style={styles.meta}>CMR {stop.cmrNumber} · POD {stop.evidenceCount}</Text>{stop.contact_phone ? <TouchableOpacity onPress={() => Linking.openURL(`tel:${stop.contact_phone}`)}><Text style={styles.phone}>{stop.contact_phone}</Text></TouchableOpacity> : <Text style={styles.warning}>Sin teléfono de contacto</Text>}</View><Text style={styles.status}>{stop.status}</Text></View>{stop.status !== "Completada" && <View style={styles.actions}><Small label={stop.arrived_at ? "Llegada ✓" : "He llegado"} onPress={() => postEvent(stop, "arrival")} /><Small label={`Foto POD (${stop.evidenceCount})`} onPress={() => addPhoto(stop)} /><Small label="Completar" onPress={() => postEvent(stop, "complete")} /></View>}</View>)}{loading && <View style={styles.loading}><ActivityIndicator /><Text>Sincronizando…</Text></View>}</ScrollView></SafeAreaView>;
}
...
function Button({ label, onPress, tone = "primary" }: { label: string; onPress: () => void; tone?: "primary" | "secondary" }) {
  return <TouchableOpacity style={tone === "primary" ? styles.button : styles.buttonSecondary} onPress={onPress}><Text style={tone === "primary" ? styles.buttonText : styles.buttonSecondaryText}>{label}</Text></TouchableOpacity>;
}
function Small({ label, onPress }: { label: string; onPress: () => void }) { return <TouchableOpacity style={styles.small} onPress={onPress}><Text style={styles.smallText}>{label}</Text></TouchableOpacity>; }
