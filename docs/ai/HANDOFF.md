...
The P0 remains open for credential scope and rotation: replace All Environments scopes with Production-only credentials or an isolated Preview resource, enable separate Production secret values, rotate affected credentials and prove the final configuration. Existing runnable Preview artifacts are removed and new privileged Preview builds are blocked; canceled metadata records generated by future branch pushes may still appear and contain no built artifact.
