...
CMR remediation is prepared in draft PR #81 at exact HEAD `5387a8d61f6d3466c99f1edc7d55ac3ba00e21fe`. It removes the fabricated initial CMR values, preserves the API invariant that issuance without an expedition fails with 422, and adds regression coverage. Local validation reports 128 tests plus typecheck passing; GitHub `validate`, both Vercel contexts and Vercel Preview Comments are green on that HEAD, while Supabase Preview is skipped.

Claude's 2026-09-21 exact-HEAD analysis found a blocking pre-existing input-path defect exposed by #81: `customerIds` and the seven ADR controls are read-only, `customerIds` is mandatory in UI and API, and current writers use incompatible aliases (`clienteId` / `cliente`) while the inheritance reader expects `customerId` / `codigoCliente` / `customerIds`. Removing demo defaults therefore makes a clean-browser canonical CMR impossible to issue. Before merge, make those eight fields editable and accept the existing writer aliases without reintroducing fabricated values. ADR label capture and canonical `hazmat_entries.label_codes` propagation remain separate because the current ADR edition is not active. No merge or production promotion was performed.
