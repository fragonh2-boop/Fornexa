...
- **Estado:** DIAGNOSTICADO; DRIFT DDL CONFIRMADO; PRODUCCIÓN SANA; REPARACIÓN SOURCE-ONLY EN CURSO; REPLAY CONTROLADO PENDIENTE
...
- **Comparación SQL corregida:** de las 30 filas estándar almacenadas como un único SQL, 16 coinciden por blob/contenido, 13 difieren y 1 es remote-only. Una revisión independiente posterior retiró el cierre **13/13**: 6 pares están confirmados como equivalentes, `tariff_engine_foundation` tenía drift DDL real y 6 pares vuelven a quedar pendientes de contraste directo.
- **Drift confirmado y alcance de la reparación:** la versión ejecutada de `tariff_engine_foundation` contiene el índice único `tariff_rules_tenant_id_id_key`, ausente en Git pero presente en producción. Es requisito de la FK compuesta tenant-aware de `pricing_run_components`; sin él, un replay limpio falla. La rama `codex/a2-restore-tariff-rule-index` restaura la sentencia idempotente y añade una prueba de orden, sin ejecutar SQL ni modificar producción/historial.
- **Pares reabiertos:** `review_access_token_registry`, `harden_review_token_rpc`, `mobile_trip_access`, `shared_party_addresses`, `shared_party_address_indexes` y `deca_native_atomic_issuance` requieren comparación directa Git-vs-statements. No inferir equivalencia desde la revisión documental de PR #71.
...
- **Siguiente paso ejecutable:** revisar e integrar la reparación source-only del índice; contrastar los seis pares reabiertos y `fornexa_operational_core`; después preparar la reconciliación no productiva con el hardening recuperado y ambos Git-only. El replay posterior sigue cost-gated.
