...
- **Fingerprint SQL:** se comparó el SQL realmente almacenado como ejecutado con los blob SHA de Git usando el algoritmo de objeto Git y tolerando solo el LF final. Resultado: 16/33 filas estándar son equivalentes por contenido, 16/33 difieren materialmente del SQL Git actual y 1/33 es remote-only. Incluso `customs_core`, `mobile_cmr` y `fornexa_operational_core`, cuyos prefijos de versión coinciden, tienen hoy contenido Git distinto del SQL ejecutado originalmente. Por tanto A2 no se puede resolver renombrando timestamps.
...
- **Siguiente paso ejecutable:** revisar semánticamente los 16 ficheros cuyo SQL Git actual difiere del SQL almacenado como ejecutado y preparar una rama de reconciliación no productiva que preserve el hardening recuperado y clasifique explícitamente los dos Git-only.
