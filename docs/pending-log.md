...
- **Estado:** DIAGNOSTICADO; PRODUCCIÓN SANA; GIT INTEGRATION `MIGRATIONS_FAILED`; REPLAY CONTROLADO PENDIENTE
- **Evidencia:** `docs/verification/supabase-migration-provenance-20260911.md`.
...
- **Comparación SQL validada:** de las 30 filas estándar almacenadas como un único SQL, 16 coinciden por blob/contenido (exactas o solo cambia el LF final), 13 difieren a nivel byte/contenido y 1 es remote-only. La revisión manual de las 13 diferencias confirmó **13/13 equivalentes a nivel de SQL ejecutable**: las diferencias observadas son comentarios, espacios, saltos de línea o maquetación de la misma sentencia, no cambios DDL/DML. Por tanto no queda drift semántico confirmado entre esos 29 pares single-string.
- **Primeras tres filas:** se almacenan como arrays de 8/20/83 sentencias, por lo que el blob hash directo no es válido. `customs_core` y `mobile_cmr` están contrastadas manualmente y son semánticamente equivalentes. `fornexa_operational_core` sigue pendiente de comparación canónica statement-a-statement; los intentos de reconstruir el blob con delimitadores artificiales se descartaron porque tampoco reproducen los blobs de las dos migraciones-control.
- **Git-only #1:** `20260812_local_storage_import.sql` no está en historial estándar pero sí figura aplicado en el ledger interno. Verificar todos sus efectos live antes de cualquier alineación solo de historial.
- **Git-only #2:** `20260818_cmr_number_sequence_resync.sql` no figura aplicado en ninguno de los dos ledgers. Producción muestra `cmr_number_seq.last_value=11` y máximo sufijo CMR 2026 persistido=3; el invariante se cumple hoy, pero no se infiere que la migración haya corrido.
...
- **Siguiente paso ejecutable:** completar la comparación canónica de `fornexa_operational_core`, verificar live todos los efectos de `local_storage_import` y después preparar una rama de reconciliación no productiva que preserve el hardening recuperado y clasifique explícitamente los dos Git-only.
- **Gate de replay:** aunque los 13 falsos positivos de contenido ya estén descartados semánticamente, sigue siendo obligatorio un replay desde base vacía/Preview por la divergencia de versiones/ledgers, el hardening remote-only, los Git-only y la ruta histórica obsoleta. Verificar esquema, RLS, funciones, Pedido↔Expediente 1:1, DeCA/FISCAL, telemetría y tests antes de diseñar cualquier reparación de historial.
