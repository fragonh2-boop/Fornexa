...
- **Still open:** variables remain targeted to All Environments and no credential rotation has occurred. Replace them with Production-only values or an isolated Preview resource, enable separate Production secret values, rotate affected credentials and prove the final scope with an end-to-end branch check.
- **Gate:** do not enable automated implementation until credential scopes and rotation are resolved. New pushes can create canceled-by-rule metadata records in `fornexa`; they contain no runnable Preview artifact and should not be mistaken for a completed build.
