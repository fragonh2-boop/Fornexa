...
- **Comparación SQL validada:** de las 30 filas estándar almacenadas como un único SQL, 16 son equivalentes al contenido Git (exactas o solo cambia el LF final), 13 difieren materialmente y 1 es remote-only. Las tres filas históricas iniciales se almacenan como arrays de 8/20/83 sentencias, por lo que un blob hash directo no es válido: `customs_core` y `mobile_cmr` ya fueron contrastadas manualmente y son semánticamente equivalentes; `fornexa_operational_core` queda pendiente de comparación canónica. Se descartó antes del merge una interpretación intermedia 16/33 vs 16/33 porque unir arrays sin sus delimitadores crea falsos mismatches.
...
- **Siguiente paso ejecutable:** revisar semánticamente los 13 ficheros con drift confirmado y completar la comparación canónica de `fornexa_operational_core`; después preparar una rama de reconciliación no productiva que preserve el hardening recuperado y clasifique explícitamente los dos Git-only.
- **Gate de replay:** ejecutar desde base vacía/Preview y verificar esquema, RLS, funciones, Pedido↔Expediente 1:1, DeCA/FISCAL, telemetría y tests antes de diseñar cualquier reparación de historial.
...
- **Criterio de cierre:** replay limpio + mapa de equivalencia/semántica + alineación de historial revisada + Git integration deja `MIGRATIONS_FAILED` sin alterar invariantes productivos.
