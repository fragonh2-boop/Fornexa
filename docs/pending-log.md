...
- **Corrección preparada:** el borrador nace vacío y al 0%; UI y `POST /api/cmr` comparten una única regla de completitud, con expedición obligatoria, viaje obligatorio cuando ese es el origen, campos legales, Customer ID y régimen ADR condicional. Customer ID y ADR pueden completarse manualmente; la herencia reconoce los formatos locales vigentes y heredados mediante un normalizador común. Cambiar ADR a No o sin declarar limpia el detalle ADR anterior. La expedición en origen Viaje ya era un invariante del endpoint, que devolvía 422 sin ella; ahora la UI lo anticipa. Los valores compuestos solo por espacios no cuentan como completos.
- **Aceptación:** tests cubren borrador vacío, flujo completo, origen viaje, ADR, entrada manual de Customer ID, los alias producidos por FORNEXA y botón de emisión fail-closed; ejecutar suite, typecheck, lint, build y memorandum diff-check. Después, CI y revisiones Claude + DeepSeek deben aprobar el mismo HEAD exacto antes de cualquier merge.
