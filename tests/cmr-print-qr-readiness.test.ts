...
test("CMR goods use a repeatable table header with document identity on continuation pages", () => {
  ...
  assert.ok(page.includes('<tr className={styles.goodsIdentity}><th colSpan={7}>CMR {cmr} · Mercancías</th></tr>'));
  assert.ok(page.includes('<tr className={styles.goodsHead}><th>6 Marcas y números</th>'));
